...
export const MobileDashboardNav: React.FC<MobileDashboardNavProps> = ({ role }) => {
  ...
  // Helper function to check if a link is active with null check for pathname
      ...
      {tabs.map((tab) => {
        ...
        return (
          <Link
            key={tab.label}
            href={tab.href}
            className={`flex flex-col items-center justify-center min-w-[60px] min-h-[44px] px-2 py-1 rounded-sm transition-all duration-300 relative group ${
              isActive
