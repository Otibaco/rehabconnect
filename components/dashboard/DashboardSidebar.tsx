  ...
  const navByRole: Record<UserRole, NavItem[]> = {
    patient: [
      {
        ...
        iconColor: "text-[#D99B26] group-hover:text-amber-300",
      },
      {
        label: "Notifications",
        ...
        iconColor: "text-[#C96A4B] group-hover:text-rose-300",
      },
      {
        label: "My Journey",
        href: "/dashboard/journey",
        icon: Compass,
        iconColor: "text-[#81A684] group-hover:text-emerald-300",
      },
      {
        label: "Consultations",
      ...
      },
      {
        ...
        icon: BookOpen,
        iconColor: "text-[#3B828E] group-hover:text-teal-300",
      },
      {
        label: "Payments",
      ...
      },
      {
        ...
        icon: User,
        iconColor: "text-[#5C6B73] group-hover:text-slate-200",
      },
    ],
    family: [
      {
    ...
    coordinator: [
      ...
      },
      {
        ...
        href: "/dashboard/consultation-room/cns_101",
        icon: Video,
        iconColor: "text-[#C96A4B] group-hover:text-rose-300",
      },
      {
        ...
        {currentNav.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              href={item.href}
              className={`flex items-center gap-3.5 px-3 py-3 rounded-sm transition-all duration-300 ease-in-out group relative overflow-hidden ${
                isActive
              ...
              <>
                ...
                <span
                  className={`font-mono text-xs transition-all duration-300 ease-in-out whitespace-nowrap overflow-hidden ${
                    collapsed
                      ? "opacity-0 max-w-0"
                      : "opacity-100 max-w-[160px] flex-1"
                  }`}
                >
                  {item.label}
                </span>
                ...
                {collapsed && (
                  <div className="absolute left-full ml-3 px-3 py-1.5 bg-[#0D0E0B] border border-[var(--gold)]/40 text-[var(--foreground)] font-mono text-xs rounded-sm whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 z-50 shadow-2xl flex items-center gap-2">
                    ...
                    {item.badge && (
                      <span
                        className={`px-1 py-0.2 font-extrabold text-[9px] rounded-xs ${badgeColorClasses[item.badgeColor || "amber"]}`}
                      >
                        {item.badge}
                      </span>
                    )}
