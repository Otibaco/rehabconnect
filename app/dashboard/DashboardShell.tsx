...
export const DashboardShell: React.FC<DashboardShellProps> = ({
  ...
  const patientGroups: SidebarGroup[] = [
    {
      ...
      items: [
        ...
        { label: 'Book Doctor Consult', path: '/patient/consultations/book', icon: Sparkles },
        { label: 'Telehealth Sessions', path: '/patient/appointment-confirmation', icon: Calendar },
        { label: 'Direct Messages', path: '/patient/messages', icon: MessageSquare, badge: '1 New' },
        { label: 'Care Resources', path: '/patient/resources', icon: BookOpen },
      ],
    },
    {
  ...
  const coordinatorGroups: SidebarGroup[] = [
    {
      groupName: 'Doctor Clinical Suite',
      items: [
        { label: 'Overview', path: '/coordinator/dashboard', icon: LayoutDashboard },
        { label: 'Patients', path: '/coordinator/patients', icon: Users, badge: '3 Active' },
        ...
        { label: 'Resources', path: '/coordinator/resources', icon: BookOpen },
        { label: 'Notifications', path: '/coordinator/notifications', icon: Bell, badge: unreadCount ? String(unreadCount) : undefined },
  ...
  const adminGroups: SidebarGroup[] = [
    ...
    },
    {
      ...
      items: [
        { label: 'Patients & Doctors', path: '/admin/users', icon: Users },
        { label: 'Online Programs & Protocols', path: '/admin/rehab-centres', icon: Layers, badge: '36 Active' },
        { label: 'Telehealth Sessions', path: '/admin/appointments', icon: Calendar },
      ],
    },
    {
  ...
  const roleBadgeText =
    role === 'admin'
      ...
      : role === 'coordinator'
      ? 'Doctor / Lead'
      : currentUser.onboardingTarget === 'family'
      ? 'Family Caregiver'
      : 'Patient Portal';
            ...
            <AnimatePresence>
              {roleSwitcherOpen && (
                ...
                >
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <Stethoscope className="w-3.5 h-3.5 text-[#3B828E]" /> Telehealth Doctor
                    </span>
              ...
              {notifOpen && (
                ...
                >
                  ...
                  <div className="pt-2 border-t border-[var(--border)] text-center">
                    <button
                      onClick={() => {
                        navigate(role === 'patient' ? '/patient/notifications' : role === 'admin' ? '/admin/notifications' : '/coordinator/dashboard');
                        setNotifOpen(false);
                      }}
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 bg-[var(--background)] min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[var(--border)]">
                ...
                <div className="flex items-center gap-1.5 text-xs text-[var(--foreground-subtle)] mb-1 flex-wrap">
                  {breadcrumbs.map((b, i) => (
                    <React.Fragment key={i}>
                      {i > 0 && <span>/</span>}
                      {b.path ? (
                        <button onClick={() => navigate(b.path!)} className="hover:text-[var(--gold)] transition-colors">
                          {b.label}
                        </button>
                      ) : (
            ...
            {role === 'admin' && (
              <button
                onClick={() => navigate('/admin/rehab-centres')}
                className="self-start sm:self-auto px-4 py-2.5 rounded-xl bg-[var(--gold)] hover:bg-[var(--gold-light)] text-black text-xs font-bold shadow-md shadow-[var(--gold)]/20 flex items-center gap-2 transition-transform active:scale-95 whitespace-nowrap min-h-[40px]"
              >
